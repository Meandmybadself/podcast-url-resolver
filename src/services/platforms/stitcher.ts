...
const sanitizeStitcherQuery = (string: string): string =>
  string.replace(/[^a-z|\s]/gi, "");
  ...
  implements IPlatformClient
{
  ...
  static async _searchForPodcast(
    title: string
  ): Promise<IStitcherPodcast | void> {
    const { data } = await BasePlatformClient.getPageData(
      `https://api.prod.stitcher.com/search/shows?query=${encodeURI(
    ...
    if (data?.shows?.length) {
      ...
      const matchingShow: IStitcherPodcast | undefined = find(
        data.shows,
        (show: IStitcherPodcast) =>
          makeSearchSafeString(show.title) === searchTitle
      );
      if (matchingShow) {
        return matchingShow;
      }
  ...
  async fetchPlatformEpisode(
    ...
    if (stitcherPodcast) {
      ...
      if (platformPodcast) {
        const { data } = await BasePlatformClient.getPageData(
          `https://api.prod.stitcher.com/search/episodes?query=${encodeURI(
            sanitizeStitcherQuery(canonicalEpisode.title)
          )}&count=100&offset=0`
        );
        if (data?.shows) {
          const episodeTitle: string = makeSearchSafeString(
            canonicalEpisode.title
          );
          const episodeMatch: IStitcherPodcast | undefined = find(
            data.episodes,
            (episode: IStitcherEpisode) =>
              makeSearchSafeString(episode.title) === episodeTitle &&
