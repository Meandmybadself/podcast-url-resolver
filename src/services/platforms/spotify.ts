...
export default class Spotify extends BasePlatformClient implements IPlatformClient {
	...
	static async getEpisode(spotifyPodcastId: string, episodeTitle: string, offset = 0): Promise<SpotifyEpisode | void> {
		...
		if (episodeSearchResponse?.items?.length) {
			const matchingEpisode = find(episodeSearchResponse.items, episode => makeSearchSafeString(episode.name) === searchEpisodeTitle);
			if (matchingEpisode) {
	...
	static async _getToken() {
		...
		const tokenIsValid: boolean = (Spotify._tokenExpiry || Number.POSITIVE_INFINITY) > nowish;
	...
	static async fetchPodcastIDByTitle(title: string): Promise<string | void> {
		...
		if (episodeSearchResponse) {
			...
			if (some(episodes)) {
				const matchingPodcast = find(episodes, episode => makeSearchSafeString(episode.name) === makeSearchSafeString(title));
				if (matchingPodcast) {
	...
	async fetchPlatformEpisode(canonicalPodcast: ICanonicalPodcast, canonicalEpisode: ICanonicalEpisode): Promise<void> {
		...
		if (!platformPodcast) {
			const platformPodcastId = await Spotify.fetchPodcastIDByTitle(canonicalPodcast.title);
			if (platformPodcastId) {
